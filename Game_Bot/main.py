...
# TOKEN EVENT - 5821887672:AAFJBPQBW1wBswzscnYPKy1mKidP9DVktx0

# TOKEN TESTING EVENT - 5821887672:AAFJBPQBW1wBswzscnYPKy1mKidP9DVktx0
...
# bot init
bot = Bot(token='5821887672:AAFJBPQBW1wBswzscnYPKy1mKidP9DVktx0')
dp = Dispatcher(bot)
...
async def stats(message):
     ...
     await bot.send_message(message.chat.id, f"{user_name}, статистика бота  📊\n🤵 | Гравців: {len(records)}", parse_mode='html')

@dp.message_handler(lambda message: message.text.lower() == 'игра')
async def process_command_1(message: types.Message):
    
    button1 = InlineKeyboardButton('🗿Камінь', callback_data = '1')
    button2 = InlineKeyboardButton('✂️Ножиці', callback_data = '2')
    button3 = InlineKeyboardButton('📄Бумага', callback_data = '3')
    buttons = InlineKeyboardMarkup().add(button1, button2, button3)
    await bot.send_message(message.chat.id, "🔎 Я готовий играть!\n Вибери предмет, щоб зв мною зіграти 🎭", reply_markup= buttons)

@dp.callback_query_handler(lambda c: c.data == '1')
async def process_callback_yes(callback: types.CallbackQuery):
    rand = random.choice(["🪨 Камінь", "✂️Ножиці", "📄Бумага"])

    await bot.delete_message(callback.message.chat.id, callback.message.message_id)
    await callback.message.answer("Я выбрал " + rand + "\nА ты выбрал 🗿Камінь")
    if rand == '🗿Камінь':
        await callback.message.answer("У нас ничія🤝")
    elif rand == '✂️Ножиці':
        await callback.message.answer("Ти виграв🥇")
    else:
        await callback.message.answer("Я виграв🥇")
...
async def process_callback_yes(callback: types.CallbackQuery):
    ...
    await bot.delete_message(callback.message.chat.id, callback.message.message_id)
    await callback.message.answer("Я вибрав " + rand + "\nА ти вибрав ✂️Ножиці")
    if rand == '🗿Камінь':
        await callback.message.answer("Я виграв🥇")
    elif rand == '✂️Ножиці':
        await callback.message.answer("У нас ничія🤝")
...
async def start_cmd(message):
    ...
    await message.reply(
        f' 👋Привіт {name1}\nЯ бот для гри в різноманітні ігри.\nТобі подарунок в розмірі 10.000$.\nТакож ти можеш додати  мене в чат для гри з друзями☺️.\n🆘 щоб дізнатися всі команди набери "Помощь"\nПриємної гри! Подякуй @iluha_sh.!За переклад💕',
                         parse_mode='html')
...
async def mute(message):
   ...
   if not message.reply_to_message:
      await message.reply("ℹ | Ця команда має бути відповідю на повідомлення!")
      return
   try:
      ...
      await message.reply('ℹ | Не вистачає аргументів!\nПример:\n<code>/мут 1 г причина</code>')
      return
   if mutetype == "г" or mutetype == "годин" or mutetype == "година":
      dt = datetime.now() + timedelta(hours=muteint)
      timestamp = dt.timestamp()
      await bot.restrict_chat_member(message.chat.id, message.reply_to_message.from_user.id, types.ChatPermissions(False), until_date = timestamp)
      await message.reply(f'👤 Адміністратор: {name1}\n🛑 Замутив: <a href="tg://user?id={message.reply_to_message.from_user.id}">{message.reply_to_message.from_user.first_name}</a>\n[⏰] Срок: {muteint} {mutetype}\n[📃]  Причина: {comment}',  parse_mode='html')
   if mutetype == "хв" or mutetype == "хвилин" or mutetype == "хвилини":
      dt = datetime.now() + timedelta(minutes=muteint)
      timestamp = dt.timestamp()
      await bot.restrict_chat_member(message.chat.id, message.reply_to_message.from_user.id, types.ChatPermissions(False), until_date = timestamp)
      await message.reply(f'👤 Адміністратор: {name1}\n🛑 Замутил: <a href="tg://user?id={message.reply_to_message.from_user.id}">{message.reply_to_message.from_user.first_name}</a>\n[⏰] Срок: {muteint} {mutetype}\n[📃] Причина: {comment}',  parse_mode='html')
   if mutetype == "д" or mutetype == "днів" or mutetype == "день":
      dt = datetime.now() + timedelta(days=muteint)
      timestamp = dt.timestamp()
      await bot.restrict_chat_member(message.chat.id, message.reply_to_message.from_user.id, types.ChatPermissions(False), until_date = timestamp)
      await message.reply(f'👤 Адміністратор: {name1}\n🛑 Замутил: <a href="tg://user?id={message.reply_to_message.from_user.id}">{message.reply_to_message.from_user.first_name}</a>\n[⏰] Срок: {muteint} {mutetype}\n[📃] Причина: {comment}',  parse_mode='html')

@dp.message_handler(commands=['розмут', 'unmute'], commands_prefix='/', is_chat_admin=True)
async def unmute(message):
   name1 = message.from_user.get_mention(as_html=True)
   if not message.reply_to_message:
      await message.reply("ℹ Ця команда має бути відповідню на повідомлення!")
      return
...
@dp.message_handler(commands=['ban', 'бан', 'кік', 'kick'], commands_prefix='/', is_chat_admin=True)
...
@dp.message_handler(commands=['розбан', 'unban'], commands_prefix='/', is_chat_admin=True)
...
@dp.message_handler(text=['Инфо','інфо'])
async def info_user(message: types.Message):
    ...
    if user_status_reply == 'Player':
        user_status_reply2 = 'Гравець'
    if user_status_reply == 'Admin':
        user_status_reply2 = 'Адміністратор'
    if user_status_reply == 'Rab':
        user_status_reply2 = 'Розробник'
...
''', parse_mode='html')
        ...
        await bot.send_message(message.chat.id, f'{name}, Доступ до даної команди закритий. Для відкритя адміністратора зверніться до  @iluha_sh🦰', parse_mode='html')

@dp.message_handler(commands=['ping', 'пинг'], commands_prefix=["/"])
async def ping(message: types.Message):
    a = time.time()
    bot_msg = await message.answer(f'⚙ Перевірка пінгу....')
    if bot_msg:
        b = time.time()
        await bot_msg.edit_text(f'🖥️ Пінг: {round((b - a) * 1000)} ms')
...
async def fff(message: types.Message):
       ...
       g = random.choice(h)
       await message.reply(f"""Шанс цього {g} """)   
       
@dp.message_handler(lambda t: t.text.startswith("Шар"))
async def fff(message: types.Message):
       h = ["Моя відповідь - ні","Мені здається - так","Зараз неможна сказати","Мені здається - ні","Знаки говорять - ні","Так","Ніт","Можеш бути упевнений в цьому"]
       g = random.choice(h)
       await message.reply(f"""{g} """)                                                                
@dp.message_handler(lambda t: t.text.startswith("Вибери"))
async def fff(message: types.Message):
       h = ["Мені здається,що 1 варіант краще","Однозначно другий","Однозначно перший","Другий варіант краще","Перший варіант краще"]
       g = random.choice(h)
       await message.reply(f"""{g} """) 
...
async def plus_rep(message):
   ...
   if message.from_user.id == message.reply_to_message.from_user.id:
      await message.reply("А неможна накручивати собі репутацію!:)")
      return
   db.UpdateUserValue('reputation', 1, message.reply_to_message.from_user.id)
   db.con.commit()
   await message.reply("Збільшення репутації засчитано👍")
...
async def minus_rep(message):
   if not message.reply_to_message:
      ...
      return
   if message.from_user.id == message.reply_to_message.from_user.id:
      await message.reply("Накручивати репутацію заборонено!Но-Но-Но😌")
      return
   db.UpdateUserValueMinus('reputation', 1, message.reply_to_message.from_user.id)
   db.con.commit()
   await message.reply("Пониження репутацію засчитано👎")               

@dp.message_handler(commands=['r'])
async def report(message: types.Message):
  try:
    if message.text == '/report' or message.text == '/r' or not message.reply_to_message:
      await bot.send_message(message.chat.id, '''Ось інформація за систему репортів ⛔️

⚠️ | Правила по использованию репортов
     [1️⃣] принижувати когось.
     [2️⃣] Капсити, писать нерозбірливо, використовувати спам, писать один и  текст пару раз.
      [3️⃣] Всяко дразнити адміністрацію и відволікати від работи.
      [4️⃣] Не чіплятись,зачіпати:@Ukrainian_girl22
      [5️⃣] Забороняється реклама в любому її виду
      [6️⃣] без попр
      [7️⃣] Заборонено винити інших.
      [8️⃣] Репорт робить по принципу - Питання/Прохання/Жалоба (виключення - Привітання) або такі. Або інші форми репорту будуть залишатися без відповіді і буде надання покарання.В Куток😂❤️.

[⚠️] | Форма відправки репорта - /report [повідомлення]

[⛔️] | Прошу вас дотримуватися правилл відправки репорта''')
    else:
      ...
      if send == 0:
        await bot.send_message(message.chat.id, '[👮] | Адміни не сповіщенні, для відправки їм репорта потрібно щоб вони   запустили мене в пп!')
      else:
        await bot.send_message(message.chat.id, '''[✅] | ваш репорт був успішно відправлений адміністрації''')
  except:
    pass
...
async def privazka(message):
  ...
  if message.chat.type == 'supergroup':
    ...
    if cdb.cursor.fetchone() == None:
      ...
      await message.reply('Ти прив'язав бота до чату 🛡')
    else:
      await message.reply('Бот вже прив'язаний до чату ✅')
  if message.chat.type == 'private':
    await message.reply('Введіть цю команду в своєму чаті 👻') 
        
@dp.message_handler(commands=['ping', 'пинг', 'пінг'], commands_prefix=["/", "!"])
async def ping(message: types.Message):
 a = time.time()
 bot_msg = await message.answer(f'Перевірка пінгу...')
 if bot_msg:
  b = time.time()
 await bot_msg.edit_text(f'Пінг бота: {round((b-a)*1000)} мс')  
...
async def prof_user(message: types.Message):
    ...
    if status_block == 'off':
        ...
        if message.text.lower() in ["купить улюбленця 1", "Купить улюбленця 1"]:    
            ...
            if checking == 1:
                await bot.send_message(chat_id, f'ℹ | Дочекайтеся закінчення гри! {rloser}', parse_mode='html')
                return
            ...
            if int(pets) == 0:
                if pet1 == 0:
                    if int(balance) >= int(summ):
                        await bot.send_message(message.chat.id, f'🐥 | {user_name}, ви успішно купили маленьку ціпу за 1.000.000$ 🎉', parse_mode='html')
                        ...
                        await bot.send_message(message.chat.id, f'💰 | {user_name}, не хватає грошей! {rloser}', parse_mode='html')
                        return
                if pet1 == 1:
                    await bot.send_message(message.chat.id, f'ℹ️ | {user_name}, у вас вже є даний улюбленець! {rloser}', parse_mode='html')     
                    return
            if pets == 1:
                await bot.send_message(message.chat.id, f'ℹ️ | {user_name}, у вас вже є улюбленець! {rloser}', parse_mode='html')     

        if message.text.lower() in ["купити улюбленця 2", "Купить питомца 2"]:    
            user_name = message.from_user.get_mention(as_html=True)
            pet1 = cursor.execute("SELECT pet1 from users where user_id = ?",(message.from_user.id,)).fetchone()
            pet1 = int(pet1[0])
            pet2 = cursor.execute("SELECT pet2 from users where user_id = ?",(message.from_user.id,)).fetchone()
        ...
        if message.text.lower() in ["продать питомца", "Продать питомца"]:
            ...
            if pets == 0:
                await bot.send_message(message.chat.id, f'ℹ️ | {user_name}, у вас нету питомца! {rloser}', parse_mode='html')
            if pet1 == 1:
                await bot.send_message(message.chat.id, f'💰 | {user_name}, вы успешно продали своего питомца за 828181818750.00000000$', parse_mode='html')
                cursor.execute(f'UPDATE users SET balance = {balance + 8281818118750000} WHERE user_id = "{user_id}"') 
                cursor.execute(f'UPDATE users SET pet1 = {pet1 - c} WHERE user_id = "{user_id}"') 
                cursor.execute(f'UPDATE users SET pet_hp = {100} WHERE user_id = "{user_id}"') 
                cursor.execute(f'UPDATE users SET pet_eat = {100} WHERE user_id = "{user_id}"') 
                cursor.execute(f'UPDATE users SET pet_mood = {100} WHERE user_id = "{user_id}"') 
        ...
        if message.text.lower() in ["баланс", "Баланс", "Б", "б"]:
            ...
            await bot.send_message(message.chat.id,
                                       ...
                                       parse_mode='html')
        ################################################КУПИТЬ Энергию######################################################
        if message.text.startswith('Купити енергію'):
            name = message.from_user.get_mention(as_html=True)
            user_id = message.from_user.id
            ...
            if ob_summ <= 10:
                if ob_summ <= balance:
                    await bot.send_message(message.chat.id,
                                           f'{name}, ви успішно купили {summ} ⚡️ за {ob_summ3}$ {rwin}',
                                           parse_mode='html')
                ...
                await bot.send_message(message.chat.id,
                                       f'{name}, неможна робити покупку Енергії більше ліміту {rloser}\nЛимит: 10 ⚡️',
                                       parse_mode='html')

        ################################################ПРОФИЛЬ#############################################################
        if message.text.lower() in ["Профіль", "Профиль"]:
            msg = message
            chat_id = message.chat.id
            name1 = message.from_user.get_mention(as_html=True)
            user_name = msg.from_user.full_name
            ...
            if besiness == 0:
                besiness2 = ''
            if besiness == 1:
                besiness2 = '💼 Бизнес: Шаурмова'
            if besiness == 2:
                besiness2 = '💼 Бизнес: Нічний Клуб'
            if besiness == 3:
                besiness2 = '💼 Бизнес: Кальянная'
            if besiness == 4:
                besiness2 = '💼 Бизнес: АЗС'
            if besiness == 5:
                besiness2 = '💼 Бизнес: Порностудія'
            if besiness == 6:
                besiness2 = '💼 Бизнес: Маленький офіс'
            if besiness == 7:
                besiness2 = '💼 Бизнес: Нафтовишка'
            if besiness == 8:
                besiness2 = '💼 Бизнес: Космічне агентство'
            if besiness == 9:
                besiness2 = '💼 Бизнес: Межпланетний експресс'
            if besiness == 10:
                besiness2 = '💼 Бизнес: Генератор матерії'
            if besiness == 11:
                besiness2 = '💼 Бизнес: Генератор матерії'
            # Телефоны
            if phone == 0:
            ...
            if house == 3:
                house2 = '🏠 Дім: Сарай'
            if house == 4:
                house2 = '🏠 Дім: Маленький дім'
            if house == 5:
                house2 = '🏠 Дім: Квартира'
            if house == 6:
                house2 = '🏠 Дім: Великий дім'
            if house == 7:
                house2 = '🏠 Дім: Коттедж'
            if house == 8:
                house2 = '🏠 Дім: Вілла'
            if house == 9:
                house2 = '🏠 Дім: Дім за містом'
            if house == 10:
                house2 = '🏠 Дім: Висотка'
            if house == 11:
                house2 = '🏠 Дім: Дім на мальдівах'
            if house == 12:
                house2 = '🏠 Дім: Технологічна висотка'
            if house == 13:
                house2 = '🏠 Дім: Свій острів'
            if house == 14:
                house2 = '🏠 Дім: Дім на марсі'
            if house == 15:
                house2 = '🏠 Дім: Острів на марсі'
            if house == 16:
                house2 = '🏠 Дім: Свій марс'
            if house == 17:
                house2 =  🏠 Дім: Своя Галактика
            # Вертолёты
            if helicopter == 0:
                helicopter2 = ''
            if helicopter == 1:
                helicopter2 = '🚁 Вертолёт: Літаючий шар'
            if helicopter == 2:
            ...
            if plane == 15:
                plane2 = '✈️ Самоліт: Ту-22м2'
            if plane == 16:
                plane2 = '✈️ Самоліт: Ту-160'
            # Машины
            if cars == 0:
                cars2 = ''
            if cars == 1:
    ...
    📦 Ваша Власність:
        ...
        {besiness2}
            ...
            if user_status[0] == 'Rab':
                user_status2 = '😋 Розробник'
                     
            if user_status[0] == 'Admin':
                user_status2 = '😛🦰 Aдміністратор' 
...
################################################ПРОФИЛЬ#############################################################
    ...
    ''', parse_mode='html')
        ...
        if message.text.lower() == 'ограбить казну':
            ...
            rx_money = random.randint(100000000000000000000, 5000000000009999999999)
    ...
    ''', parse_mode='html')
        ...
        if message.text.lower() in ['!беседа', '!Беседа']:
            ...
            await bot.send_message(message.chat.id, f'{name} 💬Официальный канал  - @FoxiModules', parse_mode='html')
        #######################################РП Команды#########################################
    ...
    ''', parse_mode='html')
        ...
        if message.text.lower() in ['Казна', 'казна']:
            ...
            await bot.send_message(message.chat.id,
                                   f'{name}, 💰 На данный момент казна штата составляет 70.326.975.785.225.897.829.929.919.999.999.999.999.999.999$',
                                   parse_mode='html')
        ##################################ЕЖЕДНЕВНЫЙ БОНУС########################################
        if message.text.lower() == 'ежедневный бонус':
            ...
            rx_money = random.randint(10000000000000000000, 50000000000000000000000000)
            rx_money2 = '{:,}'.format(rx_money)
...
🍎 1 Фермер -54,000,000,999,999$ [1lvl]
...
💵 | Деньги на вывод: 0$
        ...
        if message.text.startswith("купить самолёт"):
            ...
            if nomer == 16:
                ...
                if balance >= price:
                    if plane < 1:
                        await bot.send_message(message.chat.id,
                                               f'{name}, вы успешно купили самолёт "Ту-160" 🎉',
                                               parse_mode='html')
                        cursor.execute(f'UPDATE property SET have = "on"  WHERE user_id = "{user_id}"')
                        cursor.execute(f'UPDATE users SET balance = {balance - price}  WHERE user_id = "{user_id}"')
    ...
    ✈️ 16. Ту-160 - 13.500.000.000.000$
...
''', parse_mode='html')
       ...
       ###########################################АДМИН############################################
        ...
        if message.text.lower() in ["помощь", "Помощь"]:
            ...
            name1 = message.from_user.get_mention(as_html=True)
            await bot.send_message(message.chat.id,
                                   f'{name1}, выберите категорию:\n   1️⃣ Основное\n   2️⃣ Игры\n   3️⃣ Развлекательное\n   4️⃣ Кланы(В разработке...)\n   5️⃣Модерация чата\n   6️⃣Донат\n\n💬 Так же у нас есть общий чат https://t.me/+PRoxgKQTdDQ5OGI6\n🆘 По всем вопросам - @iluha_sh ',                  
                                   reply_markup=help, parse_mode='html')
...
async def inlinebutton(callback: types.CallbackQuery):
    await callback.message.answer('🔎 Кланы находится в разработке!')
...
🛒 За донатом обращайтесь к разработчику @iluha_sh

...
async def privazka(message):
  ...
  if message.chat.type == 'supergroup':
    ...
    if cdb.cursor.fetchone() == None:
      ...
      await message.reply('Ти привязав бота до чату 🛡')
    else:
      await message.reply('Бот вже привязаний до чату ✅')
...
################################################ПРОФИЛЬ#############################################################
    ...
    ''', parse_mode='html')
        ...
        if message.text.lower() in ['Казна', 'казна']:
            ...
            await bot.send_message(message.chat.id,
                                   f'{name}, 💰 На данный момент казна штата составляет 70.326.975.785.225.897.829.929.919.999.999.999.999.999.999999999999$',
                                   parse_mode='html')
        ##################################ЕЖЕДНЕВНЫЙ БОНУС########################################
...
🏛 |  Основной счёт: {bank2} $
💼 | Хранительный счёт: 0$
🔐 | Деньги на депозите: 9999999999999$

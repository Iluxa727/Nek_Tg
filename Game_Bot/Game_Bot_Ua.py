...
################################################ПРОФИЛЬ#############################################################
    ...
    ''', parse_mode='html')
        ...
        if message.text.lower() in ['Казна', 'казна']:
            ...
            await bot.send_message(message.chat.id,
                                   f'{name}, 💰 На данный момент казна штата составляет 70.326.975.785.225.897.829.929.919$',
                                   parse_mode='html')
        ##################################ЕЖЕДНЕВНЫЙ БОНУС########################################
...
''', parse_mode='html')
       ...
       ###########################################АДМИН############################################
        ...
        if message.text.lower() in ["помощь", "Помощь"]:
            ...
            name1 = message.from_user.get_mention(as_html=True)
            await bot.send_message(message.chat.id,
                                   f'{name1}, выберите категорию:\n   1️⃣ Основное\n   2️⃣ Игры\n   3️⃣ Развлекательное\n   4️⃣ Кланы(В разработке...)\n   5️⃣Модерация чата\n   6️⃣Донат\n\n💬 Так же у нас есть общий чат https://t.me/bmgceat\n🆘 По всем вопросам - @clekerroblox ',                  
                                   reply_markup=help, parse_mode='html')
